...
python optim/validate_batch.py --config configs/tuned/xxx.yaml --plant truck_trailer  # 并行 V1 验证（~6 min）
...
- **`optim/validate_batch.py`**：并行 49 场景 V1 验证入口。支持 `baseline vs tuned` 或自定义 A/B（比如"有 MLP vs 无 MLP"）；两次 B=49 批量仿真，总耗时 ~6 min（scalar per-scene 约 10 min）；自定义 label 避免 post_training 的"调参前/后"硬标签
