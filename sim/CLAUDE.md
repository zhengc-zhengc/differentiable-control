...
- 调用：`python optim/train_batch.py --epochs 6 --plant truck_trailer`
...
- **`run_simulation_batch(..., hard_mode=True)`**：关闭所有 smooth 近似、走硬限幅（与 scalar `run_simulation(differentiable=False)` 的 V1 路径逐元素一致，tolerance ~5mm），自动 `torch.no_grad()` 包裹。用于 V1 验证等不需要梯度的场景
- **`post_training.run_comparison(..., use_batched=True)` / `run_post_training(..., use_batched=True)`**：V1 对比环节内部切到 batched hard_mode，~6 min（scalar per-scene ~10 min）；`train_batch.py` 默认 `use_batched=True`，传 `--scalar-validation` 可退回 scalar 做回归对照
- **`optim/validate_batch.py`**：自定义 A/B 对比入口（比如"有 MLP vs 无 MLP"、"不同 tuned yaml"）；支持 `--config-a/--config-b`、自定义 label、命令行 `--disable-mlp-{a,b}` / `--trailer-mass-{a,b}` 覆盖
