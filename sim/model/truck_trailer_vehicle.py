...
从外部仓库 truckdynamicmodel 直接 import 模型代码（base_model + model_structure +
data_utils），保持源码单一所有权——仓库更新时 sim 自动跟随。
...
# 将外部 truckdynamicmodel 仓库加入 sys.path
_SIM_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_REPO_ROOT = os.path.dirname(_SIM_DIR)
_PLANT_REPO = os.path.normpath(os.path.join(
    _REPO_ROOT, '..', 'truckdynamicmodel', 'truck_trailer_residual_modular'))
if _PLANT_REPO not in sys.path:
    sys.path.insert(0, _PLANT_REPO)

# 必须放到 sys.path 设置之后
from base_model import TruckTrailerNominalDynamics, wrap_angle_error_torch  # noqa: E402
from model_structure import MLPErrorModel  # noqa: E402

# 内联 data_utils 中的 2 个纯 torch 函数，避免引入 pandas/sklearn 依赖
# 与 truckdynamicmodel/truck_trailer_residual_modular/data_utils.py 保持一致
...
class TruckTrailerVehicle:
    ...
    内部维护 12D 状态（牵引车 6 + 挂车 6），用外部 TruckTrailerNominalDynamics
