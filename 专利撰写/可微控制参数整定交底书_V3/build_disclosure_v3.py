"""Build V3 patent disclosure with CNIPA prior art and black-and-white DOCX."""
...
V2_FIG_DIR = ROOT / "专利撰写" / "可微控制参数整定交底书_V2" / "figures"
...
def draw_box(
    ...
    *,
    ...
    x1, y1, x2, y2 = xy
    draw.rounded_rectangle(xy, radius=8, fill=fill, outline="black", width=2)
    f = font(24 if title else 20, bold=title)
    lines = text.split("\n")
...
def generate_system_diagram(path: Path) -> None:
    ...
    draw.text((60, 35), "图1  可微闭环整定系统框图", fill="black", font=title_font)

    boxes = {
        "A": (70, 130, 360, 250, "原始控制器\n代码/参数表"),
        "B": (450, 130, 740, 250, "双模式控制器\n训练路径/验证路径"),
        "C": (830, 130, 1120, 250, "车辆动力学模型\n名义/参数域"),
        "D": (1210, 130, 1500, 250, "多场景轨迹库\n轨迹/速度段"),
        "E": (450, 370, 740, 500, "闭环展开模块\n横向-纵向-动力学"),
        "F": (830, 370, 1120, 500, "综合损失模块\n误差/平滑/正则"),
        "G": (1210, 370, 1500, 500, "自动微分模块\n梯度回传"),
        "H": (830, 620, 1120, 750, "参数投影模块\n物理边界约束"),
        "I": (450, 620, 740, 750, "产物生成模块\n整定配置/日志"),
        "J": (70, 620, 360, 750, "硬逻辑复验模块\n原始分支/限幅/速率"),
        "K": (1210, 620, 1500, 750, "训练增强模块\n域随机化/噪声/抖动"),
    }
    for key, (x1, y1, x2, y2, txt) in boxes.items():
        draw_box(draw, (x1, y1, x2, y2), txt, fill="#F7F7F7" if key in {"E", "F", "G"} else "white")

    arrow(draw, (360, 190), (450, 190))
    arrow(draw, (740, 190), (830, 190))
    arrow(draw, (1120, 190), (1210, 190))
    arrow(draw, (595, 250), (595, 370))
    arrow(draw, (740, 435), (830, 435))
    arrow(draw, (1120, 435), (1210, 435))
    arrow(draw, (1355, 500), (975, 620))
    arrow(draw, (830, 685), (740, 685))
    arrow(draw, (450, 685), (360, 685))
    arrow(draw, (215, 620), (215, 250))
    arrow(draw, (360, 190), (450, 435))
    arrow(draw, (1355, 620), (1120, 500))
    arrow(draw, (1355, 250), (1355, 370))

    note_font = font(18)
    draw.text((70, 900), "说明：训练路径保留可微近似用于梯度更新，验证路径保留工程硬逻辑用于部署前复验。", fill="black", font=note_font)
    img.save(path)
...
def generate_flow_diagram(path: Path) -> None:
    ...
    draw = ImageDraw.Draw(img)
    draw.text((60, 35), "图2  控制器参数自动整定流程图", fill="black", font=font(34, bold=True))
    steps = [
        ("S1", "读取控制器逻辑\n识别参数与边界"),
        ("S2", "构造双模式控制器\n可微训练/硬逻辑验证"),
        ("S3", "构造车辆动力学\n建立物理参数域"),
    ...
    for idx, (sid, label) in enumerate(steps):
        ...
        draw_box(draw, (x, y, x + w, y + h), f"{sid}\n{label}", title=True if idx in {0, 7} else False)
    for i in range(3):
        arrow(draw, (coords[i][2], (coords[i][1] + coords[i][3]) // 2), (coords[i + 1][0], (coords[i + 1][1] + coords[i + 1][3]) // 2))
    arrow(draw, ((coords[3][0] + coords[3][2]) // 2, coords[3][3]), ((coords[7][0] + coords[7][2]) // 2, coords[7][1]))
    for i in range(7, 4, -1):
        arrow(draw, (coords[i][0], (coords[i][1] + coords[i][3]) // 2), (coords[i - 1][2], (coords[i - 1][1] + coords[i - 1][3]) // 2))
    # feedback loop when validation fails
    draw.line([(coords[7][2], (coords[7][1] + coords[7][3]) // 2), (1660, 765), (1660, 1040), (285, 1040), (285, 555)], fill="black", width=2)
    arrow(draw, (285, 555), (285, coords[4][3]))
    draw.text((1180, 1010), "复验未达标则继续整定", fill="black", font=font(20))
    img.save(path)


def grayscale_copy(src: Path, dst: Path) -> None:
    img = Image.open(src).convert("RGB")
    img = ImageOps.grayscale(img)
    # Keep grayscale but return RGB for DOCX compatibility.
    ImageOps.autocontrast(img).convert("RGB").save(dst)
...
def generate_figures() -> None:
    ...
    mapping = {
        "fig3_loss_curve.png": "fig3_loss_curve.png",
        "fig4_parameter_changes.png": "fig4_parameter_changes.png",
        "fig5_training_summary.png": "fig5_training_summary.png",
        "fig6_comparison_trajectory.png": "fig6_comparison_trajectory.png",
        "fig7_comparison_lateral_error.png": "fig7_comparison_lateral_error.png",
    }
    for dst_name, src_name in mapping.items():
        grayscale_copy(V2_FIG_DIR / src_name, FIG_DIR / dst_name)
...
def docx_markdown(md: str) -> str:
    """Build a temporary markdown for Pandoc: diagrams become images, not code."""
    system_block = SYSTEM_MERMAID.strip()
    flow_block = FLOW_MERMAID.strip()
    md = md.replace(system_block, "![图1 可微闭环整定系统框图](figures/fig1_system_architecture.png)")
    md = md.replace(flow_block, "![图2 控制器参数自动整定流程](figures/fig2_method_flow.png)")
    return md
...
def style_docx(path: Path) -> None:
    ...
    heading_specs = {
        "Heading 1": ("黑体", 15, True, 12, 6),
        "Heading 2": ("黑体", 13.5, True, 10, 5),
        "Heading 3": ("黑体", 12, True, 8, 4),
        "Heading 4": ("黑体", 11.5, True, 6, 3),
    }
    for style_name, (font_name, size, bold, before, after) in heading_specs.items():
    ...
    for char_style in ["Hyperlink", "FollowedHyperlink"]:
        if char_style in styles:
            styles[char_style].font.color.rgb = RGBColor(0, 0, 0)
            styles[char_style].font.underline = False
    ...
    for para in doc.paragraphs:
        if para.text.strip().startswith("图") or any(run._element.xpath(".//w:drawing") for run in para.runs):
            para.alignment = WD_ALIGN_PARAGRAPH.CENTER

    # Tables: black borders, white cells, comfortable padding.
        ...
        for row_idx, row in enumerate(table.rows):
            for cell in row.cells:
                ...
                for p in cell.paragraphs:
                    ...
                    for run in p.runs:
                        set_run_font(run, "宋体", 10.5, True if row_idx == 0 else None)

    # Image sizing: constrain large figures to page width.
    max_width = Cm(15.4)
...
def patch_docx_ooxml(path: Path) -> dict[str, int]:
    """Remove residual blue/theme coloring and count Office Math objects.

    Keep the original OOXML structure intact. Some DOCX readers are pickier
    after full XML serialization, especially around Office Math namespaces.
    """
    ns = {
    ...
    with zipfile.ZipFile(path, "r") as zin, zipfile.ZipFile(tmp, "w", zipfile.ZIP_DEFLATED) as zout:
        for item in zin.infolist():
            ...
            if item.filename in {"word/document.xml", "word/styles.xml"}:
                for color in [b"2E74B5", b"1F4D78", b"0563C1", b"0000FF"]:
                    data = data.replace(color, b"000000")
                data = re.sub(rb'\s+w:themeColor="[^"]+"', b"", data)
                data = re.sub(rb'\s+w:themeTint="[^"]+"', b"", data)
                data = re.sub(rb'\s+w:themeShade="[^"]+"', b"", data)
    ...
    return {
        "math_omath": len(root.findall(".//m:oMath", ns)),
        "math_para": len(root.findall(".//m:oMathPara", ns)),
        "blue_colors": doc_xml.count(b"2E74B5") + doc_xml.count(b"1F4D78"),
    }
...
def build(timestamp: str | None = None) -> tuple[Path, Path, dict[str, int]]:
    ...
    stats = patch_docx_ooxml(docx_path)
