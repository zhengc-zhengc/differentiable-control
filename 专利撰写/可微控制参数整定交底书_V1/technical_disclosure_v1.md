...
推荐发明名称：一种基于车辆动力学可微闭环仿真的车辆横纵向控制器参数自动整定方法
...
| 序号 | 名称建议 | 适用策略 |
| --- | --- | --- |
| 首选 | 一种基于车辆动力学可微闭环仿真的车辆横纵向控制器参数自动整定方法 | 保护范围聚焦，能够覆盖项目当前最有技术效果的闭环整定流程。 |
...
![图1 车辆横纵向控制器可微闭环参数整定总体流程](figures/fig1_overall_flow.png)

![图2 可微训练路径与硬逻辑验证路径](figures/fig2_dual_path.png)

![图3 多轨迹多域并行鲁棒整定流程](figures/fig3_batched_domain_randomization.png)

![图4 实施例一训练损失曲线](figures/fig4_loss_curve.png)

![图5 实施例一控制器参数变化图](figures/fig5_parameter_changes.png)

![图6 实施例一训练摘要和硬逻辑验证统计图](figures/fig6_training_summary.png)

![图7 实施例一调参前后轨迹跟踪对比图](figures/fig7_comparison_trajectory.png)

![图8 实施例一调参前后横向误差对比图](figures/fig8_comparison_lateral_error.png)
